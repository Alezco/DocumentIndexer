...
/*
    public List<Term> createTermList(final String content) {
    ...
    */
    ...
    public float frequency(final String content) {
    ...
    public domain.Document index(final String input) {
        ...
        int nbWords = cleanup.split("\\s+").length;
        ...
        while (it.hasNext()) {
            ...
            int occurence = countsOccurence.get(key); // tu peux typer plus finement ici
            float frequency = (float)(occurence / nbWords);
            // Positions
            List<Integer> positions = new ArrayList<>();
            String[] strings = cleanup.split("\\s+");
            for (int i = 0; i < strings.length; i++)
            {
                // Comment matcher les mot qui ont été stemmé ??
                if (strings[i].equals(key.toString()))
                    positions.add(i);
            }

            Term term = new Term(key.toString(), positions, frequency);
            terms.add(term);
        }
